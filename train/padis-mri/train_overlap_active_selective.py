...
def main(**kwargs):
    ...
    active_patch_sizes = parse_int_list(opts.active_patch_sizes)
    if opts.overlap_mode == 'independent':
        if len(active_patch_sizes) == 0:
            raise click.ClickException('--active-patch-sizes cannot be empty')
        if opts.patch_list is not None:
            patch_set = set(parse_int_list(opts.patch_list))
            active_set = set(active_patch_sizes)
            missing = active_set - patch_set
            if missing:
                raise click.ClickException(
                    f'active_patch_sizes={sorted(active_set)} contains sizes not in patch_list={sorted(patch_set)}'
                )
    ...
    if opts.precond == 'vp':
        ...
        if opts.overlap_mode == 'same':
            ...
            # 仅在 active_patch_sizes 指定的 patch 尺度上启用独立噪声、
            # 中心引导的 overlap loss。未指定尺度由 loss 类内部回退到
            # 原始 Patch_EDMLoss，用于保持公平对照。
            c.loss_kwargs.class_name = (
                'training.patch_overlap_independent_selective_loss.'
    ...
    if opts.overlap_mode == 'same':
        ...
        desc += f'-overlap-independent-center-lam{lambda_tag}-p{active_tag}'
