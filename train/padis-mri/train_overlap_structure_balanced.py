...
"""Train PaDIS-MRI balanced gradient/wavelet overlap experiments."""
...
def float_tag(value):
    """Convert a float to a path-safe experiment tag."""
...
@click.option(
    '--padding',
    help='Zero padding for training',
    metavar='BOOL',
    type=bool,
    default=True,
    show_default=True,
...
@click.option(
    '--four_channels',
    help='Number of Fourier embedding freqs',
    metavar='INT',
    type=int,
    default=1,
    show_default=True,
...
@click.option(
    '--hash_channels',
    help='Number of hash embedding outputs',
    metavar='INT',
    type=int,
    default=1,
    show_default=True,
...
@click.option(
    '--pad_width',
    help='Width on all sides of zero padding',
    metavar='INT',
    type=int,
    required=True,
)
@click.option(
    '--patch-list',
    help='Comma-separated patch sizes, e.g. 96,192,384',
    type=str,
)
@click.option(
    '--patch-probs',
    help='Comma-separated probabilities for patch sizes, e.g. 0.2,0.3,0.5',
    type=str,
)
# Main options.
@click.option(
...
@click.option(
    '--lambda-overlap',
    help='Maximum overlap auxiliary loss weight',
    type=click.FloatRange(min=0),
    default=0.01,
    show_default=True,
)
@click.option(
    '--structure-mode',
    help='Structural overlap component',
    type=click.Choice(['gradient', 'wavelet']),
    required=True,
)
@click.option(
    '--structure-mix',
    help='Mixture ratio of the scale-aligned structural gradient',
    type=click.FloatRange(min=0, max=1),
    default=0.5,
    show_default=True,
)
@click.option(
    '--structure-scale-min',
    help='Minimum detached structure scale',
    type=click.FloatRange(min=0, min_open=True),
    default=0.1,
    show_default=True,
)
@click.option(
    '--structure-scale-max',
    help='Maximum detached structure scale',
    type=click.FloatRange(min=0, min_open=True),
    default=10.0,
    show_default=True,
)
@click.option(
    '--min-noise-ratio',
    help='Minimum valid diff-mode noise ratio',
    type=float,
    default=1.25,
    show_default=True,
)
@click.option(
    '--max-noise-ratio',
    help='Maximum valid diff-mode noise ratio',
    type=float,
    default=3.0,
    show_default=True,
)
...
def main(**kwargs):
    """Train a diffusion-based generative model."""
    ...
    if opts.precond == 'vp':
        ...
        if opts.overlap_mode == 'same':
            c.loss_kwargs.class_name = (
                'training.patch_overlap_loss.SameNoiseOverlapPatch_EDMLoss'
            )
            ...
            c.loss_kwargs.class_name = (
                'training.patch_overlap_loss.DifferentNoiseOverlapPatch_EDMLoss'
            )
            c.loss_kwargs.update(
                P_mean=-1.2,
                P_std=1.2,
            ...
            # Only the 64x64 branch uses balanced structure-aware CG-OL.
            # Smaller patches fall back to the original Patch_EDMLoss.
            c.loss_kwargs.class_name = (
                'training.patch_overlap_independent_structure_balanced_loss.'
                'IndependentNoiseStructureOverlapPatchEDMLoss'
            )
            c.loss_kwargs.update(
                P_mean=-1.2,
                P_std=1.2,
                ...
                structure_mode=opts.structure_mode,
                structure_mix=opts.structure_mix,
                structure_scale_min=opts.structure_scale_min,
                structure_scale_max=opts.structure_scale_max,
            )

        else:
            raise click.ClickException(
                f'Unsupported overlap mode: {opts.overlap_mode}'
            )
    else:
    ...
    if opts.seed is not None:
        ...
        seed = torch.randint(
            1 << 31, size=[], device=torch.device('cuda')
        )
    ...
    lambda_tag = float_tag(opts.lambda_overlap)

    if opts.overlap_mode == 'same':
        desc += f'-overlap-same-lam{lambda_tag}'
    elif opts.overlap_mode == 'diff':
        min_ratio_tag = float_tag(opts.min_noise_ratio)
        max_ratio_tag = float_tag(opts.max_noise_ratio)
        desc += (
            f'-overlap-diff-lam{lambda_tag}'
            f'-r{min_ratio_tag}-{max_ratio_tag}'
        )
    elif opts.overlap_mode == 'independent':
        desc += (
            f'-overlap-structure-{opts.structure_mode}'
            f'-mix{float_tag(opts.structure_mix)}'
            f'-lam{lambda_tag}-p64'
        )
    ...
    if dist.get_rank() != 0:
        ...
        c.run_dir = os.path.join(
            opts.outdir, f'{cur_run_id:05d}-{desc}'
        )
    ...
    dist.print0(f'Max overlap loss weight: {opts.lambda_overlap}')
    if opts.overlap_mode == 'independent':
        dist.print0('Active overlap patch:    64')
        dist.print0(f'Structure mode:          {opts.structure_mode}')
        dist.print0(f'Structure mixture:       {opts.structure_mix}')
        dist.print0(
            f'Structure scale range:   '
            f'[{opts.structure_scale_min}, {opts.structure_scale_max}]'
        )
    ...
    if dist.get_rank() == 0:
        ...
        with open(
            os.path.join(c.run_dir, 'training_options.json'), 'wt'
        ) as f:
