...
@click.option(
    ...
    type=click.Choice(['same', 'diff', 'independent', 'iac_independent']),
    required=True,
)
@click.option('--lambda-overlap', help='Overlap auxiliary loss weight', type=float, default=0.01, show_default=True)
@click.option('--anatomy-fg-threshold', help='Foreground threshold for internal-aware overlap loss', type=float, default=0.05, show_default=True)
@click.option('--anatomy-erosion-radius', help='Erosion radius for defining internal foreground region', type=int, default=8, show_default=True)
@click.option('--anatomy-internal-weight', help='Overlap consistency weight inside eroded foreground', type=float, default=1.0, show_default=True)
@click.option('--anatomy-outer-weight', help='Overlap consistency weight in outer foreground band', type=float, default=0.2, show_default=True)
@click.option('--anatomy-bg-weight', help='Overlap consistency weight in background', type=float, default=0.0, show_default=True)
...
def main(**kwargs):
    ...
    if opts.precond == 'vp':
        ...
        elif opts.overlap_mode == 'iac_independent':
            # Internal-aware CG-OL：保留 independent center-guided overlap 逻辑，
            # 但降低 outer contour / background 对辅助一致性项的贡献。
            c.loss_kwargs.class_name = (
                'training.patch_overlap_independent_iac_loss.'
                'IndependentNoiseInternalAwareOverlapPatch_EDMLoss'
            )
            c.loss_kwargs.update(
                P_mean=-1.2, P_std=1.2, sigma_data=0.5,
                lambda_overlap=opts.lambda_overlap,
                active_patch_size=64,
                anatomy_fg_threshold=opts.anatomy_fg_threshold,
                anatomy_erosion_radius=opts.anatomy_erosion_radius,
                anatomy_internal_weight=opts.anatomy_internal_weight,
                anatomy_outer_weight=opts.anatomy_outer_weight,
                anatomy_bg_weight=opts.anatomy_bg_weight,
            )
    ...
    if opts.overlap_mode == 'same':
        ...
        outer_tag = str(opts.anatomy_outer_weight).replace('.', 'p')
        erode_tag = str(opts.anatomy_erosion_radius)
        desc += f'-iacgol-lam{lambda_tag}-erode{erode_tag}-outer{outer_tag}-p64'
    ...
    if opts.overlap_mode in ['independent', 'iac_independent']:
        dist.print0('Active overlap patch:    64')
    if opts.overlap_mode == 'iac_independent':
        dist.print0(f'Anatomy fg threshold:    {opts.anatomy_fg_threshold}')
        dist.print0(f'Anatomy erosion radius:  {opts.anatomy_erosion_radius}')
        dist.print0(f'Anatomy internal weight: {opts.anatomy_internal_weight}')
        dist.print0(f'Anatomy outer weight:    {opts.anatomy_outer_weight}')
        dist.print0(f'Anatomy bg weight:       {opts.anatomy_bg_weight}')
