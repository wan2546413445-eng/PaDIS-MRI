...
@click.option(
    '--overlap-sigma-center',
    help='Sigmoid center sigma',
    type=click.FloatRange(min=0, min_open=True),
    default=0.2,
    show_default=True,
)
@click.option(
    '--overlap-sigma-slope',
    help='Sigmoid slope in log-sigma space',
    type=click.FloatRange(min=0, min_open=True),
    default=2.0,
    show_default=True,
)
...
def main(**kwargs):
    ...
    if opts.precond == 'vp':
        ...
        if opts.overlap_mode == 'same':
            ...
            c.loss_kwargs.class_name = (
                'training.patch_overlap_independent_sigma_aware_loss.'
            ...
            c.loss_kwargs.update(
                ...
                overlap_sigma_slope=opts.overlap_sigma_slope,
                overlap_sigma_low=opts.overlap_sigma_low,
                overlap_sigma_high=opts.overlap_sigma_high,
            )
    ...
    if opts.overlap_mode == 'same':
        ...
        # fixed 模式沿用旧命名，便于完全复现实验目录和历史脚本。
        desc += f'-overlap-independent-center-lam{lambda_tag}-p64'
        if opts.overlap_weight_mode == 'sigmoid':
            desc += (
                f'-wsig-c{float_tag(opts.overlap_sigma_center)}'
                f'-k{float_tag(opts.overlap_sigma_slope)}'
                f'-bins{float_tag(opts.overlap_sigma_low)}'
                f'-{float_tag(opts.overlap_sigma_high)}'
            )
        elif opts.overlap_weight_mode == 'piecewise':
            desc += (
    ...
    if opts.overlap_mode == 'independent':
        ...
        dist.print0(
            f'Overlap sigma slope:     {opts.overlap_sigma_slope}'
        )
        dist.print0(
            f'Overlap sigma bins:      '
            f'low<={opts.overlap_sigma_low}, '
            f'high>={opts.overlap_sigma_high}'
        )
