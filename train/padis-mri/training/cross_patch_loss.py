...
def patchify_set(images, patch_size=64, K=8, local_k=3, global_k=4, debug=False):
    ...
    for b in range(B):
        ...
        while len(coords)<K and tried<400:
            tried+=1
            gi=int(torch.randint(0,max_i+1,(1,),device=device)); gj=int(torch.randint(0,max_j+1,(1,),device=device))
...
class CrossPatch_EDMLoss:
    def __init__(self,P_mean=-1.2,P_std=1.2,sigma_data=0.5,cp_k=8,cp_local_k=3,cp_global_k=4,cp_patch_size=64,cp_debug=False):
