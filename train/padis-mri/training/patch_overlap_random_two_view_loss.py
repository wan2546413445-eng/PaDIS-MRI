"""Random two-view overlap EDM training without AGG and OC."""
...
class RandomIndependentNoiseOverlapPatch_EDMLoss(Patch_EDMLoss):
    """Random two-view EDM training without AGG guidance or OC."""
    ...
    def __call__(
        ...
        # Random two-view baseline:
        # keep two overlapping views and EDM training,
        # but remove AGG-guided patch selection.
        resolution = int(images.shape[2])
        max_offset = max(1, patch_size // 2)

        top1 = torch.randint(
            0, max(1, resolution - patch_size + 1),
            (pair_batch,), device=images.device
        )
        left1 = torch.randint(
            0, max(1, resolution - patch_size + 1),
            (pair_batch,), device=images.device
        ...
        horizontal = torch.randint(
            0, 2, (pair_batch,), device=images.device
        ).bool()
        ...
        left2 = torch.where(
            horizontal,
            (left1 + patch_size - shift).clamp(max=resolution - patch_size),
            left2,
        )

        top2 = torch.where(
            ~horizontal,
            (top1 + patch_size - shift).clamp(max=resolution - patch_size),
            top2,
        )

        overlap = patch_size - shift
