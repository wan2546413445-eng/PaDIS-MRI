...
class IndependentNoiseOverlapPatch_EDMLoss(Patch_EDMLoss):
    ...
    def __call__(
        ...
        pos1 = _position_maps(
            top1, left1, pair_batch, int(patch_size), resolution, images.device
        )
        pos2 = _position_maps(
            top2, left2, pair_batch, int(patch_size), resolution, images.device
        )
        ...
        denoised_pair = net(
            noisy_pair, sigma_pair, x_pos=pos_pair,
            class_labels=labels_pair, augment_labels=None,
        )
        ...
        d1, d2 = _boundary_distances(
            horizontal, int(patch_size), overlap, images.device, images.dtype
        )
        interior_is_1 = d1 >= d2
        D_interior = torch.where(interior_is_1, D1_overlap, D2_overlap)
        D_boundary = torch.where(interior_is_1, D2_overlap, D1_overlap)

        # 距离差越大，内部/边界角色越明确，辅助权重越大。
        # 按每个 pair 的平均值归一化，使 confidence 的平均值约为 1，
        # 避免空间权重把辅助损失整体再次压小。
        ...
        # 避免较不可靠的边界预测反向拖动内部预测。
        aux_pixel = weight * confidence * (
            D_boundary - D_interior.detach()
        ).square()

        # 每个 pair 先在通道和 overlap 空间上求平均，再扩展为 loss map。
        # training_loop 会对返回张量执行 loss.sum()，这种写法使 ratio
