...
"""Train PaDIS-MRI with simplified AGG and overlap consistency.

All patch scales use anatomy-gradient guided sampling.  The largest configured
patch scale additionally uses half-shift overlap pairs and independent noise.
"""
...
@click.option('--lambda-oc', help='Overlap-consistency loss weight',
              type=click.FloatRange(min=0), default=0.3, show_default=True)
@click.option('--overlap-mode',
              help='Overlap training mode: agg_oc, agg_no_oc, or random_two_view',
              type=click.Choice(['agg_oc', 'agg_no_oc', 'random_two_view']),
              default='agg_oc', show_default=True)
...
def main(**kwargs):
    ...
    if opts.patch_list is not None:
        ...
        active_patch_size = max(c.patch_list)
    else:
        active_patch_size = 64 if opts.padding else 384
    ...
    if opts.precond == 'vp':
        ...
        # The largest configured patch scale carries the overlap objective.
        if opts.overlap_mode == 'random_two_view':
            c.loss_kwargs.class_name = (
                'training.patch_overlap_random_two_view_loss.'
                'RandomIndependentNoiseOverlapPatch_EDMLoss'
            )
            overlap_lambda = 0.0
            overlap_rho = 0.0
            overlap_gamma = 0.0
            ...
            overlap_lambda = 0.0 if opts.overlap_mode == 'agg_no_oc' else opts.lambda_oc
            overlap_rho = opts.agg_rho
            overlap_gamma = opts.agg_gamma
        ...
        c.loss_kwargs.update(
            ...
            sigma_data=0.5,
            agg_rho=overlap_rho,
            agg_gamma=overlap_gamma,
            lambda_oc=overlap_lambda,
            active_patch_size=active_patch_size,
        )
    ...
    lambda_tag = str(opts.lambda_oc).replace('.', 'p')
    rho_tag = str(opts.agg_rho).replace('.', 'p')
    gamma_tag = str(opts.agg_gamma).replace('.', 'p')
    desc += (
        f'-agg-overlap-rho{rho_tag}'
        f'-gamma{gamma_tag}'
        f'-oc{lambda_tag}'
        f'-mode{opts.overlap_mode}'
        f'-p{active_patch_size}'
    )
    ...
    dist.print0('Training mode:           simplified AGG + overlap consistency')
    dist.print0(f'AGG mixture weight:      {opts.agg_rho}')
    dist.print0(f'AGG gradient weight:     {opts.agg_gamma}')
    dist.print0(f'Overlap loss weight:     {opts.lambda_oc}')
    dist.print0(f'Active overlap patch:    {active_patch_size}')
