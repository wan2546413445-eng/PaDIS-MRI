...
# Global-context options.  First version follows "local patches + downsampled global context":
# the denoiser sees [noisy_patch, x_pos, y_pos, lowres_global_context_patch].
@click.option('--global_context_mode',
              type=click.Choice(['magnitude_lowres', 'complex_lowres', 'none']),
              default='magnitude_lowres', show_default=True,
              help='Global context channel type.')
@click.option('--global_context_size', type=click.IntRange(min=8), default=96, show_default=True,
              help='Downsampled global context size before upsampling back to image size.')
@click.option('--global_context_dropout', type=click.FloatRange(min=0, max=1), default=0.0, show_default=True,
              help='Drop full global-context condition per sample during training.')
@click.option('--global_context_from', type=click.Choice(['noisy', 'clean']), default='noisy', show_default=True,
              help='Build context from current noisy image x_t or clean target. Use noisy for train/eval consistency.')
...
def main(**kwargs):
    ...
    # We reuse the existing PaDIS x_pos concatenation path.
    # training_loop interprets hash_channels != 1 as:
    #     net_input_channels = image_channels + hash_channels
    ...
    if opts.precond == 'vp':
        ...
        c.loss_kwargs.global_context_dropout = opts.global_context_dropout
        c.loss_kwargs.global_context_from = opts.global_context_from
    else:
