...
class NumpyFolderDataset(Dataset):
    def __init__(self,
        ...
        data            = None,  # Data to pass through the network.
        **super_kwargs,         # Additional arguments for the Dataset base class.
    ):
        data_path = path
        self._path = "/".join(data_path.split("/")[0:-1]) + "/32dB/"
        self._use_pyspng = use_pyspng
        print("\nLoading Dataset from: " + str(data_path))
        data = torch.load(path + "/noisy.pt")
        self._data = data
        self._zipfile = None
        ...
        print(self._data["x_est"].shape)
        self._image_fnames = np.arange(self._data["x_est"].shape[0])
    ...
    def _load_raw_image(self, raw_idx):
        ...
        # image = np.array(torch.view_as_real(self._data["x_est"][idx]).permute(2, 0, 1))
        image = np.array(torch.view_as_real(self._data["x_est"][raw_idx]).permute(2, 0, 1))
        return image
...
#----------------------------------------------------------------------------
# Same exact format as the ImageFolderDataset except passing noisy inputs
# through the network for denoising.
...
class NoisyFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,          # Additional arguments for the Dataset base class.
        ...
        self._use_pyspng = use_pyspng
        print("\n-->Loading Dataset from: " + str(data_path))
        data = torch.load(data_path + "/noisy.pt")
        self._data = data
        ...
        print(f"self_path: {self._path}")
        print(f"output: {os.path.isdir(self._path)}")
        if os.path.isdir(self._path):
            self._type = 'dir'
            self._all_fnames = {os.path.relpath(os.path.join(root, fname), start=self._path) for root, _dirs, files in os.walk(self._path) for fname in files}
        elif self._file_ext(self._path) == '.zip':
            ...
            raise IOError('APath must point to a directory or zip')
        
        print("done")
        # self._image_fnames = sorted(fname for fname in self._all_fnames if self._file_ext(fname) == ".npy") #NOTE changed to only grab .npy
        # if len(self._image_fnames) == 0:
        #     raise IOError('BNo image files found in the specified path')
