...
# Patch diagnostic runner: uses evaluator_patchdiag instead of evaluator.
from evaluator_patchdiag import DPSHyperEvaluator
from utils import post_eval_normalize


def parse_args():
    p = argparse.ArgumentParser(description="PaDIS patch-boundary diagnostic runner")
    ...
    p.add_argument("--zeta", type=float, default=3.0, help="Chosen zeta value for padis/edm")
    p.add_argument("--steps", type=int, default=104, help="Number of steps or ADMM iterations")
    p.add_argument(
        "--inner_loops",
        type=int,
        default=10,
    ...
    p.add_argument("--gpus", type=int, nargs="+", default=None, help="GPU ids visible inside the process, e.g. --gpus 0")
    ...
    p.add_argument("--save_intermediate", action="store_true", help="Save intermediate PaDIS recon figures and metrics")
    p.add_argument("--intermediate_every", type=int, default=10, help="Save intermediate diagnostics every N outer steps")

    # Patch-boundary diagnostic options.
    p.add_argument("--boundary_stat", action="store_true", help="Enable boundary/interior magnitude-MSE diagnostics")
    p.add_argument("--boundary_every", type=int, default=10, help="Boundary-stat interval in outer diffusion steps")
    p.add_argument("--boundary_width", type=int, default=4, help="Patch boundary band width in pixels")
    p.add_argument("--boundary_fg_threshold", type=float, default=0.05, help="Foreground threshold as ratio of GT max magnitude")

    # Optional aggregation ablation. Default keeps original hard residual stitching.
    p.add_argument(
        "--patch_agg",
        type=str,
        default="hard",
        choices=["hard", "edge_atten"],
        help="Patch residual aggregation ablation. hard keeps original PaDIS behavior.",
    )
    p.add_argument("--edge_sigma", type=float, default=0.6, help="Gaussian-window sigma for edge_atten")
    p.add_argument("--edge_floor", type=float, default=0.5, help="Minimum residual weight at patch edges for edge_atten")
...
def configure_patchdiag_env(args):
    """Configure environment-variable switches used by denoise_padding_patchdiag.py."""
    if args.patch_agg == "edge_atten":
        os.environ["PADIS_PATCH_AGG"] = "edge_atten"
        os.environ["PADIS_EDGE_SIGMA"] = str(args.edge_sigma)
        os.environ["PADIS_EDGE_FLOOR"] = str(args.edge_floor)
        print(
            f"[PatchDiag] PADIS_PATCH_AGG=edge_atten | "
            f"PADIS_EDGE_SIGMA={args.edge_sigma} | PADIS_EDGE_FLOOR={args.edge_floor}"
        )
    else:
        # Keep original hard residual stitching unless explicitly requested.
        os.environ.pop("PADIS_PATCH_AGG", None)
        os.environ.pop("PADIS_EDGE_SIGMA", None)
        os.environ.pop("PADIS_EDGE_FLOOR", None)
        print("[PatchDiag] PADIS_PATCH_AGG=hard | original residual hard stitching")


def main():
    args = parse_args()
    os.makedirs(args.save_dir, exist_ok=True)
    configure_patchdiag_env(args)
    ...
    if args.run_evaluate:
        metrics = opt.evaluate(
            ...
            boundary_fg_threshold=args.boundary_fg_threshold,
        )
        s = metrics['summary']
