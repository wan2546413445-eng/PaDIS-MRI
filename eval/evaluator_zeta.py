...
from recon_zeta import dps2, dps_uncond, dps_edm, dps_uncond_edm
...
class DPSHyperEvaluator:
    ...
    def _load_sample_for_bart(self,
        ...
        if seed_ind is not None:
            r_idx = self.R_levels.index(self.mask_select)
            mask2d = data['masks'][r_idx, seed_ind].to(device)
            mask_t = mask2d[None, None, ...]
        else:
            chosen = self.mask_select if mask_ind is None else mask_ind
            mask_t = data[f"mask_{chosen}"][None, ...].to(device)
            mask2d = mask_t.squeeze(0).squeeze(0)

        ksp_us_torch = mask_t * fs_ksp
        ksp_und = (
            ksp_us_torch.squeeze(0).permute(1, 2, 0)
    ...
    def _build_latents_and_pos(self, pad_override: int = None) -> None:
        """(Re)build `latents` and `latents_pos` whenever psize/pad change."""
        ...
        self.latents_pos = pos.unsqueeze(0)

    def dps_edm_wrapper(self,
    ...
    def sweep_patch_sizes(
        ...
        with open(csv_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["psize", "idx", "psnr", "ssim", "nrmse"])
            writer.writeheader()
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for mask_id in mask_list:
                ...
                for i, idx in enumerate(subset, start=1):
                    ...
                    np.save(
                        os.path.join(recons_dir, f"recon_{tag_label}.npy"),
                        recon.cpu().numpy()
                    )
    ...
    def _evaluate_zeta(self, zeta: float, num_steps: int, subset: list[int]) -> float:
        ...
        for idx in subset:
            meas, gt, invop = self._load_sample(idx)
            _, _, recon_psnr, _, _, _, _ = self.dps2_wrapper(
                invop, meas, gt, zeta, self.pad, self.psize, num_steps
            )
