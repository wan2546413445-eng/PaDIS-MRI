...
class DPSHyperEvaluator:
    def __init__(self,
        ...
        # Prefer repeatability over speed for validation.
    ...
    def _set_sample_seed(
        ...
        """
        Reset every RNG at the beginning of every validation sample.

        Fixed-per-sample policy:
            sample 0  -> seed = self.seed
            sample 1  -> seed = self.seed
            ...
            sample 31 -> seed = self.seed

        Therefore, evaluating sample 3 alone with seed=123 and evaluating
        sample 3 inside a 32-sample run both start from exactly seed=123.

        ``idx`` is retained only for logging and API compatibility. It does not
        alter the seed. ``stream_offset`` may be used deliberately for an
        additional stochastic repeat, e.g. 0, 1, 2.

        For exact reproducibility, use one GPU. Python ``random`` and NumPy RNG
        states are process-global and are unsafe to reseed concurrently from
        multiple worker threads.
        """
        sample_seed = int(self.seed) + int(stream_offset)
    ...
    def _reset_sample_latent(
            self,
            gpu_id: int = None,
    ) -> torch.Tensor:
        """
        Recreate the initial diffusion latent after the sample RNG has been
        reset. This is required because the latent created in ``__init__`` is
        generated before ``evaluate()`` starts and otherwise is not controlled
        by the per-sample reset.
        """
        if torch.cuda.is_available():
            if gpu_id is None:
                device = self.device
    ...
    def evaluate(
        ...
        # Exact single-sample/full-set agreement requires serial execution.
        # Python random and NumPy RNG are process-global, so thread workers can
        # overwrite one another's seed. In strict mode, keep only one GPU.
        if strict_reproducible and len(gpus) > 1:
            print(
                "[Reproducibility] strict_reproducible=True: "
                f"using only GPU {gpus[0]} instead of {gpus}."
            )
            gpus = [gpus[0]]
        ...
        def worker(subset, gpu_id):
            ...
            if algo.lower() != "admm" and self.model is not None:
                if gpu_id is not None and torch.cuda.is_available():
                    ...
                    self.latents_pos = self.latents_pos.cpu()
            ...
            for i, idx in enumerate(subset, start=1):
                ...
                # Important: regenerate x_T only after resetting seed.
                # Every sample starts from the same seed-controlled latent and
                # the same subsequent random-number stream.
                if algo.lower() != "admm":
                    self._reset_sample_latent(gpu_id=gpu_id)

                print(
                    f"[Seed] sample={idx} | "
                    f"reconstruction_seed={sample_seed} | "
                    "policy=fixed_per_sample"
                )
        ...
        if len(gpus) == 1:
            # No worker thread is needed. Serial execution removes RNG races.
            worker(splits[0], gpus[0])
        else:
            threads = []
            for gpu, subset in zip(gpus, splits):
                t = threading.Thread(target=worker, args=(subset, gpu))
        ...
        metrics = {
            ...
            'summary': summary,
            'rng_policy': {
                'mode': 'fixed_per_sample',
                'seed': int(self.seed),
                'strict_reproducible': bool(strict_reproducible),
                'gpus_used': gpus,
            },
        }
