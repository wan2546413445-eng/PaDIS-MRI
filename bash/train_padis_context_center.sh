...
ROOT_OUTDIR=$RESULT_ROOT/PaDIS-MRI-runs/context_center_training-runs_default
...
LOG_FILE=$LOG_DIR/train_context_center_default_${RUN_NAME}_gpu${GPU}_${TIME_TAG}.log
...
{
  echo "=================================================="
  echo "Context-Center PaDIS-MRI Training"
  echo "Sigma route: ORIGINAL DEFAULTS"
  echo "MODE=$MODE"
...
CUDA_VISIBLE_DEVICES=$GPU torchrun --standalone --nproc_per_node=$NPROC train/padis-mri/train_context_center.py \
  --outdir=$OUTDIR \
  --data=$DATA_DIR \
  --cond=0 \
  --arch=ddpmpp \
