...
WORKERS=4
SEED=123
EXP_NAME=overlap_diff_lam001_r125_r300_s16s32s64_p020305_b2_seed123
MODE=${1:-debug}
...
LOG_FILE=$LOG_DIR/train_overlap_diff_${RUN_NAME}_gpu${GPU}_${TIME_TAG}.log
cd "$CODE_ROOT"
{
  echo "Overlap Different-Noise PaDIS-MRI Training"; echo "MODE=$MODE"; echo "RUN_NAME=$RUN_NAME"; echo "GPU=$GPU"; echo "OUTDIR=$OUTDIR"; echo "DATA=$DATA_DIR";
} | tee "$LOG_FILE"
CUDA_VISIBLE_DEVICES=$GPU torchrun --standalone --nproc_per_node=$NPROC train/padis-mri/train_overlap.py \
  ...
  --overlap-mode=diff --lambda-overlap=0.01 --min-noise-ratio=1.25 --max-noise-ratio=3.0 \
