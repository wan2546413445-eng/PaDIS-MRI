...
OVERLAP_MODE=${OVERLAP_MODE:-agg_oc}
...
OC_TAG=${LAMBDA_OC//./p}
RUN_NAME=main_agg_overlap_rho${RHO_TAG}_gamma${GAMMA_TAG}_oc${OC_TAG}_mode${OVERLAP_MODE}_pad${PAD_WIDTH}_seed${SEED}
...

{
    ...
    echo "AGG_RHO=$AGG_RHO AGG_GAMMA=$AGG_GAMMA LAMBDA_OC=$LAMBDA_OC OVERLAP_MODE=$OVERLAP_MODE"
    echo "OUTDIR=$OUTDIR"
...
CUDA_VISIBLE_DEVICES="$GPU" torchrun \
    --standalone \
    --nproc_per_node=1 \
    train/padis-mri/train_overlap_agg_v2_mode.py \
    --outdir="$OUTDIR" \
    --data="$DATA_DIR" \
    --cond=0 \
    --arch=ddpmpp \
