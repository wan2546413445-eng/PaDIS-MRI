...
parser.add_argument('--noise_level', type=str, default="32dB", choices=["32dB", "22dB", "12dB"],
                    help='Noise level to add')
parser.add_argument('--acs_size', type=int, default=24, help='Number of ACS lines to use')
parser.add_argument('--h5_folder', type=str, default="/data/datasets/fastmri/multicoil_val",
                    help='Path to input folder containing .h5 files')
parser.add_argument('--output_root', type=str, default="/data/datasets/fastmri/",
                    help='Path to output folder to save results')
parser.add_argument('--contrast', type=str, default="t1-flair", choices=["t1-flair", "t2"],
                    help='Contrast to filter for')
...
if args.only_file is not None:
    ksp_files = [f for f in ksp_files if os.path.basename(f) == args.only_file]
    if len(ksp_files) != 1:
        raise ValueError(f"Cannot uniquely find {args.only_file}, got {len(ksp_files)} matches: {ksp_files}")
...
for i in tqdm(range(total_iterations)):
    ...
    gt_img_white_cropped = sp.resize(bart(1, 'pics -S -i 30', ksp_white[:, :, None, :], s_maps_white[:, :, None, :]),
                                     [imsize, imsize])
    ...
    norm_const_99_white = normalization_const(
        s_maps_white,
    ...
    gt_img_white_cropped = sp.resize(bart(1, 'pics -S -i 30', ksp_white.transpose(1, 2, 0)[:, :, None, :],
                                          s_maps_white.transpose(1, 2, 0)[:, :, None, :]), [imsize, imsize])
    cimg_white = bart(1, 'fft -iu 3', ksp_white.transpose(1, 2, 0)).transpose(2, 0,
                                                                              1)  # compare to `bart fft -iu 3 ksp cimg`
    ...
    save_name = args.out_name if args.out_name is not None else (
        f'sample_{tag}_{i}.pt' if args.contrast == "t1-flair" else f'sample_{i}.pt'
    )

    if save_name.startswith("sample_"):
        noise_name = save_name.replace("sample_", "noise_var_", 1)
    else:
        noise_name = f"noise_var_{i}.pt"
    ...
    torch.save({'gt': torch.tensor(gt_img_white_cropped, dtype=torch.complex64),
                ...
                'source_file': ksp_files[idx],
                'source_h5_name': fname,
                'source_slice': slice_idx,
                'imsize': imsize,
                'acs_size': ACS_size},
               os.path.join(path, save_name))

    torch.save({"noise_var_noisy": var},
               os.path.join(path, noise_name))

    with open(os.path.join(path, "mapping.txt"), "a") as f:
        f.write(f"{save_name} <- {fname}, slice={slice_idx}, imsize={imsize}, acs={ACS_size}\n")