...
for i in tqdm(range(total_iterations)):
    ...
    torch.save({'gt': torch.tensor(gt_img_white_cropped, dtype=torch.complex64),
               ...
               os.path.join(path, f'sample_{tag}_{i}.pt') if args.contrast == "t1-flair" else os.path.join(path,
                                                                                                           f'sample_{i}.pt'))

    torch.save({"noise_var_noisy": var},
               os.path.join(path, f'noise_var_{tag}_{i}.pt') if args.contrast == "t1-flair" else os.path.join(path,
                                                                                                              f'noise_var_{i}.pt'))