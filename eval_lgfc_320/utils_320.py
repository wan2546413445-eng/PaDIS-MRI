...
def _nrmse(gt: np.ndarray, recon: np.ndarray) -> float:
    ...
    return float(np.linalg.norm((gt - recon).ravel()) / max(denom, 1e-12))
...
def _center320(array: np.ndarray) -> np.ndarray:
    ...
    raise ValueError(f"Expected 320x320 or 384x384 reconstruction, got {array.shape[-2:]}")
...
def post_eval_normalize_320(
    ...
    """Evaluate reconstructions and GT on the identical central 320 ROI.

    The normalization constant is still derived from each original fully
    sampled 384 k-space, exactly as in the existing author-style post-eval.
    This function also accepts a 384 baseline reconstruction and center-crops
    it to 320, which enables a fair same-ROI baseline comparison.
    """
    ...
    for path in recon_files:
        ...
        psnr = float(peak_signal_noise_ratio(gt, recon, data_range=data_range))
        ssim = float(structural_similarity(gt, recon, data_range=data_range))
        ...
        print(
            f"[320 ROI] sample {idx}: PSNR={psnr:.3f}, "
    ...
    json_path = output_dir / (
        json_basename if json_basename.endswith(".json") else f"{json_basename}.json"
    )
