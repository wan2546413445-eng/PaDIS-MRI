...
def denoised_from_patches_320(
    ...
    state (rather than the injected noisy state) avoids putting VE noise into
    the 384 x0 estimate outside the central 320 target.
    """
    for name, value in (
        ("x_noisy_real", x_noisy_real),
        ("x_state_real", x_state_real),
    ):
        if value.ndim != 4 or value.shape[-2:] != (CANVAS_SIZE, CANVAS_SIZE):
            raise ValueError(
                f"{name} must end in {CANVAS_SIZE}x{CANVAS_SIZE}, "
    ...
    # Start from zero padding, then restore the original 384 measurement FOV
    # with identity x0 outside the central target.
