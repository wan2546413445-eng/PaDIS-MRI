...
class Lgfc320DPSHyperEvaluator(DPSHyperEvaluator):
    ...
    def _build_latents_and_pos(self, pad_override: int = None) -> None:
        ...
        # The latent only defines the PaDIS target size / patch grid.
        self.latents = torch.randn(
            [1, 1, 320, 320],
            device=self.device,
        )

        # Preserve the exact positional coordinate system learned at training:
        # original 384 target + 64 padding per side -> 512 coordinate grid.
        ...
        # 448 inference canvas is exactly the central crop [32:480] of the
        # original 512 training/inference canvas.
