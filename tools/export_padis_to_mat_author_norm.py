...
def normalization_const_from_ksp_author(ksp: np.ndarray) -> np.float32:
    """
    Match the author's renormalizer.py logic:
        fs_ksp = fftmod(data['ksp'])
        ACS_size = 20
        ksp_acs_only = sp.resize(sp.resize(fs_ksp, (Nc, 20, 20)), fs_ksp.shape)
        ACS_img = sp.rss(sp.ifft(ksp_acs_only, axes=(-2, -1)), axes=(0,))
        norm_const = np.percentile(abs(ACS_img), 99)

    Input ksp is raw data['ksp'] with shape [Nc, H, W].
    """
    fs_ksp = fftmod_np(ksp)
    num_coils, H, W = fs_ksp.shape
    acs_size = 20
    ksp_acs_only = sp.resize(sp.resize(fs_ksp, (num_coils, acs_size, acs_size)), fs_ksp.shape)
    acs_img = sp.rss(sp.ifft(ksp_acs_only, axes=(-2, -1)), axes=(0,))
    return np.asarray(np.percentile(np.abs(acs_img), 99), dtype=np.float32)
...
def export_one(
    ...
    # 严格按作者 renormalizer.py：从 fully-sampled k-space 的 ACS 低频图计算 99% 归一化常数
    norm_const = normalization_const_from_ksp_author(ksp)
