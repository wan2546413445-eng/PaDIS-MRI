...
def denoisedFromPatches_globalcond(
    ...
    for start in range(0, patches, patch_batch_size):
        end = min(start + patch_batch_size, patches)
        x_chunk = x_input[start:end]
        pos_chunk = pos_input[start:end]
        if use_checkpoint:
            def run_net(xc, pc):
                return net(xc, t_hat, pc, class_labels)
            out_chunk = checkpoint(run_net, x_chunk, pos_chunk, use_reentrant=False).to(torch.float64)
        else:
            out_chunk = net(x_chunk, t_hat, pos_chunk, class_labels).to(torch.float64)

        for local_i, i in enumerate(range(start, end)):
            z = indices[i]
            x_patch = x_hat[0, :, z[0]:z[1], z[2]:z[3]]
            output[0, :, z[0]:z[1], z[2]:z[3]] += out_chunk[local_i, :, :, :]
